...
class BinaryClassifier():
	...
	def classifier_train_average_perceptron(self):
		...
		for i in range (self.t):
			...
			for j in range (len(self.train_list_word)):
				...
				yhat_t = y_t * np.dot(self.w,self.train_list_word[j][0:128])
	...
	def classifier_test(self):
		...
		for j in range(len(self.test_list_word)):
			yhat_t = np.dot(self.w, self.test_list_word[j][0:128])
			# print yhat_t
			# check for mistake
...
def main():
	...
	average_mistakes_train = np.mean(average_mistakes_train, axis = 0)

	# average_mistakes_train = np.subtract(len(bc.train_list_word), average_mistakes_train)
	# average_mistakes_train = np.divide(average_mistakes_train, len(bc.train_list_word))

	# average_mistakes_test = np.mean(average_mistakes_test, axis = 0)
	# average_mistakes_test = np.subtract(len(bc.test_list_word), average_mistakes_test)
	# average_mistakes_test = np.divide(average_mistakes_test, len(bc.test_list_word))
	...
	plt.ylabel(" Total Mistakes", fontsize = 15)
	plt.title("Online Learning Curve", fontsize = 25)
	plt.grid(True)
	plt.legend(['PA', 'Perceptron'])
	plt.show()  
